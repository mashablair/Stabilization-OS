...
describe("tab separation: no overlap between Stabilizer and Builder", () => {
  ...
  it("zero tasks appear in both tabs", () => {
    ...
    const builderIds = new Set(
      tasks
        .filter((t) => t.domain === "BUSINESS" && t.status !== "DONE")
        .map((t) => t.id)
    );
...
describe("stabilizer scoring respects urgency and category weights", () => {
  it("LEGAL scores higher than MONEY (base weight)", () => {
    const legal = scoreTask(
      makeTask({ categoryId: "cat-legal" }),
      "LEGAL",
      120
    );
    const money = scoreTask(
      makeTask({ categoryId: "cat-money" }),
      "MONEY",
      120
    );
  ...
  it("MONEY scores higher than MAINTENANCE", () => {
    ...
    const maint = scoreTask(
      makeTask({ categoryId: "cat-maint" }),
      "MAINTENANCE",
      120
    );
  ...
  it("task due within 3 days scores higher than same task due in 10 days", () => {
    ...
    expect(scoreTask(soon, "LEGAL", 120)).toBeGreaterThan(
      scoreTask(later, "LEGAL", 120)
    );
  });
  ...
  it("task due within 7 days scores higher than undated task", () => {
    ...
    expect(scoreTask(dated, "LEGAL", 120)).toBeGreaterThan(
      scoreTask(undated, "LEGAL", 120)
    );
  });
  ...
  it("IN_PROGRESS gets momentum bonus", () => {
    ...
    expect(scoreTask(inProg, "LEGAL", 120)).toBeGreaterThan(
      scoreTask(backlog, "LEGAL", 120)
    );
  });
  ...
  it("small tasks (<=15 min) get higher momentum bonus than 30 min", () => {
    ...
    expect(scoreTask(small, "LEGAL", 120)).toBeGreaterThan(
      scoreTask(medium, "LEGAL", 120)
    );
  });
  ...
  it("moneyImpact adds to score", () => {
    ...
    expect(scoreTask(withMoney, "LEGAL", 120)).toBeGreaterThan(
      scoreTask(without, "LEGAL", 120)
    );
  });
});
